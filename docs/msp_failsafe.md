...
В ветке `v4_simulator_gps` read-only канал также разбирает `MSP_RAW_GPS`.
GPS считается пригодным только при наличии fix и не менее четырёх спутников.
Сырые оси магнитометра дают диагностический курс `atan2(Y, X)`; до отдельной
проверки калибровки он не используется как готовая команда разворота.
